...
public class SourceFoldersCreator {
    ...
    public List<SourceFolder> createSourceFolders(final EclipseClasspath classpath) {
        ...
        List<SourceFolder> sourceFolders = projectRelativeFolders(classpath.getSourceSets(), classpath.getTestSourceSets().getOrElse(Collections.emptySet()), provideRelativePath, classpath.getDefaultOutputDir());
    ...
    private List<SourceFolder> projectRelativeFolders(Iterable<SourceSet> sourceSets, Collection<SourceSet> testSourceSets, Function<File, String> provideRelativePath, File defaultOutputDir) {
        ...
        for (SourceSet sourceSet : sortedSourceSets) {
            ...
            for (DirectoryTree tree : sortedSourceDirs) {
                ...
                if (dir.isDirectory()) {
                    ...
                    addSourceSetAttribute(sourceSet, folder, testSourceSets);
                    entries.add(folder);
                }
            }
        }
    ...
    private void addSourceSetAttribute(SourceSet sourceSet, SourceFolder folder, Collection<SourceSet> testSourceSets) {
