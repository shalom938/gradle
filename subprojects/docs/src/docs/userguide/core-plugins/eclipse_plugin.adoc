...
[[sec:test-sources]]
Separation of test and production classpath entries
...
Although not perfect, this feature allows the Eclipse plugin to define better separation for test and non-test sources.

The Eclipse classpath consists of classpath entries: source directories, jar files, project dependencies, etc.
Each classpath entry can have a a list of classpath attributes, which are string key-value pairs.
...
The meaning of those classpath attributes are as follows:

- source directory: if `test=true` then it contains test sources otherwise it contains production sources. Production sources don't have access to test sources but test sources do have access to production sources.
- jar file: if `test=true` then it is visible for test sources only.
- project dependency: `test=true` means the classes in the target project is visible for test sources only. If `without_test_sources=false` then the test sources of the target project are also accessible.

The following rules apply for the test attribute declaration when generating the Eclipse classpath:

- Jar files and project dependencies have the `test=true` attribute if they are present only in test dependency configuration. By default a dependency configuration is a test dependency configuration if its name contains the 'test' substring (case ignored) or if it's declared via the JVM test suite DSL.
