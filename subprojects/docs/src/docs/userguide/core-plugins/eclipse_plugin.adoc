...
[[sec:test-sources]]
- Gradle has concept of source sets and dependency configuration
- Eclipse has one classpath per project concept which implies certain limitation on how can be a Gradle project mapped to Eclipse
- Eclipse 4.8 introduced the concept of test sources https://www.eclipse.org/eclipse/news/4.8/jdt.php#jdt-test-sources
- Although not perfect, the main/test source set separation mapping can be handled in a better way

Eclipse concepts:
- Classpath consists of classpath entries: source directories, jar files, project dependencies, etc.
- Each classpath entry can have a a list of classpath attributes, each attribute has a key-value pair
- source directories can have test attribute with values true or false. False means the directory contains production sources, true means the directory contains test sources.
- Production sources don't have access to test sources but test sources do have access to production sources.
- jars can have the same test classpath attributes with similar meaning as with the source directories: test=true means the jar visible only for the test sources.
- project dependencies have two attributes test and without_test_sources.
- The test attribute behaves similarly as with the jar dependencies. If test=true, then the dependent project can reference only classes in the target project from the test sources
- If without_test_sources is false, then the dependent project has visibility on the test directories of the target project. The default value is true.

Mapping Gradle projects:
- jar dependencies: marked as test if the jar is present only in (resolvable) test dependency configurations
- test dependency configurations: configurations with names that contain the substring test (case ignored) plus compile and runtime classpath configurations declared in jvm test suite DSL
- project dependencies: marked as test if the dependency is present only in (resolvable) test dependency configurations
- without_test_sources is false for project dependencies if the target project applies the java-test-fixtures plugin

Customization
- eclipse.classpath.containsTestFixtures: if set to true then all project dependencies targeting the project will have the without_test_sources=true classpath attribute
- eclipse.classpath.testSourceSets defines all source sets that is mapped to test source directories in the generated Eclipse classpath
- eclipse.classpath.testConfigurations defines all configurations that is mapped to test dependencies in  the generated Eclipse classpath

Scope:
- This section applies to both the Eclipse buildship plugin and to the built-in `eclipse` plugin
