...
=== `eclipse` plugin changed how test classpath attributes are generated

Gradle 7.3 improves the generated Eclipse classpath with regards to test sources.
Project dependencies have the `test=true` classpath attribute if they are defined in in a test configuration.
Also, the test attribute is assigned for dependencies defined in the JVM test suites.
For more information, refer t the <<eclipse_plugin.adoc#sec:test-sources, eclipse plugin documentation>>.
...
=== Deprecated `Provider#forUseAtConfigurationTime`
